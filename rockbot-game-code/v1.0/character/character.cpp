...
void character::charMove() {
    ...
    //if (is_player()) std::cout << "%%%%%% _fractional_move_speed: " << _fractional_move_speed << ", _fractional_move_speed_counter: " << _fractional_move_speed_counter << std::endl;
    ...
    if (_fractional_move_speed > 0) {
        if (_fractional_move_speed_counter >= _fractional_move_speed) { // reached the time to move an extra pixel due to fractional move_speed
            _fractional_move_speed_counter = 1;
            temp_move_speed++;
            //if (is_player()) std::cout << "** inc_move_frac **" << std::endl;
        }
	...
	if (state.animation_type == ANIM_TYPE_HIT) {
        if (hit_moved_back_n < _hit_move_back_dist) {
            ...
            temp_move_speed = 1.0;
            bool res_gravity = gravity(false);
            //std::cout << "HIT - res_gravity: " << res_gravity << std::endl;
...
bool character::gravity(bool boss_demo_mode=false)
{
    ...
    if (state.jump_state == NO_JUMP && can_fly == false && position.y < RES_H+TILESIZE+1 + frameSize.height) {
		...
		for (int i=adjusted_speed; i>0; i--) {
            ...
            //if (is_player() && state.animation_type == ANIM_TYPE_TELEPORT) std::cout << "CHAR::gravity y[" << position.y << "], i[" << i << "] - _teleport_minimal_y: " << _teleport_minimal_y << ", mapLock: " << mapLock << std::endl;

			if (state.animation_type == ANIM_TYPE_TELEPORT && position.y < _teleport_minimal_y-TILESIZE) {
                //if (is_player()) std::cout << "CHAR::gravity i[" << i << "] - not reached[" << position.y <<"] teleport_minimal[" << _teleport_minimal_y << "], keep falling" << std::endl;
