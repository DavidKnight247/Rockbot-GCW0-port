...
void common::fill_object_combo(QComboBox* combo)
{
    ...
    for (int i=0; i<GAME_MAX_OBJS; i++) {
        QString temp_str = QString("[") + QString::number(i+1) + QString("] - ") + QString(game_data.objects[i].name);
        combo->addItem(temp_str);
	}
}
...
void common::fill_weapons_names_combo(QComboBox *combo)
{
    combo->clear(); // delete all previous entries
    for (int i=0; i<MAX_WEAPON_N; i++) {
        QString temp_str = QString("[") + QString::number(i+1) + QString("] - ") + QString(game_data.weapons[i].name);
        combo->addItem(temp_str);
    }
}
